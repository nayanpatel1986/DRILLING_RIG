...
the full 14-register map (Pump + Hook Load + Spare + Rotary Performance).
Run:  docker exec -u root drillbit_backend python /app/patch_daq10_v3.py
"""
import sys
...
def patch():
    ...
    try:
        ...
        roles = ["→ HookLoad", "→ WOB", "→ SPM1", "→ SPM2", "→ PumpStrokes1",
                 "→ PumpStrokes2", "→ StandpipePressure", "→ TotalStrokes", "→ TotalSPM", 
                 "(spare)", "→ RPM", "→ Torque", "→ rap", "→ Pipe Torque"]
        regs = db.query(ModbusRegister).filter(ModbusRegister.device_id == device.id).order_by(ModbusRegister.id).all()
        for i, r in enumerate(regs):
            print(f"  [{i+1}] {r.field_name:12s} addr={r.address:6d}  {roles[i] if i < len(roles) else ''}")
